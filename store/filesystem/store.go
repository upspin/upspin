...
func (s *server) Get(ref upspin.Reference) ([]byte, []upspin.Location, error) {
	...
	pathName := upspin.PathName(ref) // The ref is the complete path name.
	...
	if parsed.User() != s.user.UserName() {
		return nil, nil, errors.E(op, errors.Invalid, parsed.Path(), errors.Errorf("mismatched user name %q", parsed.User()))
	}
