...
type server struct {
	...
	perm    *perm.Store      // permission control for the server context.
...
// New returns a StoreServer that serves the for a server context with the
// provided options.
...
func (s *server) Put(data []byte) (*upspin.Refdata, error) {
	...
	if !s.perm.IsAllowedMutation(s.user) {
		err := errors.E(op, s.user, errors.Permission, errors.Errorf("user not authorized"))
		log.Printf("Permission error: %s", err)
		return nil, err
	}
