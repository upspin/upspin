...
const (
	...
	ConfigTemporaryDir = "configTemporaryDir"
)
...
func (s *server) innerGet(userName upspin.UserName, ref upspin.Reference) (file *os.File, location upspin.Location, err error) {
	...
	location.Endpoint.Transport = upspin.GCP             //upspin.HTTPS           // Go fetch using the provided link.
	location.Endpoint.NetAddr = upspin.NetAddr(url.Host) // Important so we can ping it.
