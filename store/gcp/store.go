...
func (s *StoreClient) Get(location upspin.Location) ([]byte, []upspin.Location, error) {
	...
	default:
		log.Fatal("Can't figure out protocol")
	}
	httpReq, err := http.NewRequest("GET", request, nil)
	if err != nil {
		return nil, nil, err
	}
	resp, err := s.client.Do(httpReq)
	if err != nil {
		return nil, nil, StoreError{fmt.Sprintf("httpGet failed: %v", err)}
	}
	...
	answerType := resp.Header.Get("Content-Type")
	switch answerType {
	...
	default:
		...
		log.Printf("Content type: %v", answerType)
		return body, nil, nil
	}
	// NOT REACHED
}
...
func (s *StoreClient) Put(ref upspin.Reference, data []byte) (upspin.Location, error) {
	...
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/put", s.addr.String()), &body)
	if err != nil {
		return loc, err
	}
