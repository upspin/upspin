...
// allowUnauthenticatedConnection allows this client to connect to servers not using TLS.
const allowUnauthenticatedConnection = true
...
func (*remote) Dial(context *upspin.Context, e upspin.Endpoint) (upspin.Service, error) {
	...
	case strings.HasPrefix(addr, "http://"): // TODO: Should this be, say "grpc:"?
		conn, err := grpcauth.NewGRPCClient(e.NetAddr[7:], allowUnauthenticatedConnection)
		if err != nil {
			return nil, err
		}
