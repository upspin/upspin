...
func TestGetError(t *testing.T) {
	...
	expected := fmt.Sprintf("HTTPS Store: Get: %s: %s", ref, errSomethingBad)
...
func TestPut(t *testing.T) {
	...
	const expectedError = "upspin.Put not defined for HTTPS"
...
func TestDelete(t *testing.T) {
	...
	const expectedError = "upspin.Delete not defined for HTTPS"
	if !strings.Contains(err.Error(), expectedError) {
		t.Errorf("Expected error %s, got %s", expectedError, err)
	}
	mock.Verify(t)
