...
func (s *Store) Put(data []byte) (upspin.Reference, error) {
	return "", errors.New("upspin.Put not defined for HTTPS")
}

// Delete implements Store.
func (s *Store) Delete(ref upspin.Reference) error {
	return errors.New("upspin.Delete not defined for HTTPS")
}
...
func (s storeError) Error() string {
	if s.ref != "" {
		return fmt.Sprintf("HTTPS Store: %s: %s: %s", s.op, s.ref, s.error)
	}
	return fmt.Sprintf("HTTPS Store: %s: %s", s.op, s.error)
}
