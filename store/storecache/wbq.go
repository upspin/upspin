...
// flushed will be closed when it happenws.
type flushRequest struct {
	upspin.Location
	flushed chan bool
}

// endpointQueue reprsents a queue of pending requests destined
// for an endpoint.
...
// isWritebackFile returns false if this is not a writeback file.
// It is called by the cache start up code for each file found when walking
// the cache directories.
//
// This is used to populate the writeback queues on start up.
func (wbq *writebackQueue) isWritebackFile(path string) bool {
	...
	wbq.request <- &request{upspin.Location{Reference: upspin.Reference(elems[2]), Endpoint: *e}, nil, nil}
	return true
}
...
func (wbq *writebackQueue) flush(loc upspin.Location) {
	...
	wbq.flushRequest <- &flushRequest{loc, flushed}
	<-flushed
}
