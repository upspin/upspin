...
func (p *parallelism) success() {
	...
	// We are trying find a maximal p.max that will guarantee that p.max
	...
	// that is, the preallocated writers and hope that will be enough.
	// Unlimited would easily DOS the server.
	//
	// If we simultaneously start p.max writerbacks and they all terminate
	// with out a timeout, that would certainly prove that p.max was at or
	// lower than the maximum attainable. This heuristic here approximates
	// that. In the case of a continuous queue of writebacks, it is measuring
	// exactly that. However if thde offered load is fluctuating so that
	...
	// load only sporadically increase to p.max.
