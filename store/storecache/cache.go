...
func (c *storeCache) get(cfg upspin.Config, ref upspin.Reference, e upspin.Endpoint) ([]byte, []upspin.Location, error) {
	...
	const serviceUnavailable = "503" // String repserentation of HTTP StatusServiceUnavailable code

	// If we only see serviceUnavailable errors, retry in the hope we can live through it.
