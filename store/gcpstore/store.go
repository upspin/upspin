...
func (s *Store) Get(ref upspin.Reference) ([]byte, []upspin.Location, error) {
	...
	case "application/json":
		...
		loc, err := jsonmsg.LocationResponse(body)
		if err != nil {
...
func (s *Store) Put(data []byte) (upspin.Reference, error) {
	...
	// Parse the response
	ref, err := jsonmsg.ReferenceResponse(respBody)
	if err != nil {
...
func init() {
	...
	bind.RegisterStore(upspin.GCP, New("", httpauth.NewAnonymousClient(&http.Client{})))
}
