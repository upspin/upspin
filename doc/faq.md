...
another service, or to make an item publicy accessible when the
...
This approach, at least conceptually, requires a single key server.
With multiple key servers, names could be ambiguous.

We are aware of the problems of a single point of failure that a
single key server represents, but feel that with suitable
engineering the risk can be mitigated.

The key server is a very simple service, and the system could
easily replace it with another implementation.
...
If you write your own Upspin program and on on startup it fails with an an error like,
...
```
import "upspin.io/key/transports"
```
