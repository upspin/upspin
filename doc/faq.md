...
For simplification, let's discuss the commonest case here,
creating a file to be read by the owner alone.

When a file is created, it is encrypted using AES (TODO: LINK)
...
to the friend.
Then, the owner of the file recovers the file key, just as in
the case of reading the file, and encrypts another copy using the _friend's
public_ key. The owner then saves both encryptions of the file key in the
...
If the file is encrypted, as described above,
