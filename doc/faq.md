...
### How is user authentication done? {#authentication}
...
we expect most users to employ end-to-end encryption—EE packing
in Upspin terminology—to store their data.
...
that neither the directory server, the store server—nothing in the network—sees
the unencrypted contents of the file.

### How does encryption work? {#encryption-mechanism}

When a file is created, it is encrypted using the AES (TODO: LINK)
encryption with a unique "file key" that is generated by a random
process at the time of creation of the file.
AES is a _symmetric encryption_ technology that uses the same key
to decrypt as to encrypt.

The file key is stored in the directory entry for the file,
but that key is itself encrypted.

That second level of encryption is done using an elliptic curve algorithm (TODO: LINK),
...
To share a file with another user, say a friend, the first step is to use
access control to grant permission to the friend.
Then, it is necessary to make the file key available to the friend
to be able to share the file's contents.

To do this, the owner of the file recovers the
file key by reading it from the file's directory entry
and decrypting the file key using the owner's _private_ key, just
as when reading the file.

Next, the owner encrypts the file key again, but this time using
the _friend's public_ key.
The owner then saves both encryptions of the file key in the
directory entry for the file.
Thus, the directory entry contains an encrypted file key for
each user (in this case, owner and friend) with the right to read the file.

When the friend wants to read the file, it's the same procedure as for the
owner: get the encrypted file key from the directory entry, decrypt that, and
...
If the file is encrypted, which is almost always the case,
there must also be a copy of the file key, encrypted with
the user's public key, stored in the directory entry.
...
and therefore provides a second, very powerful level
...
### Why do I have keys rather than a password? {#no-password}
