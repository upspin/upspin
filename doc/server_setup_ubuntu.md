...
we will manually grant the `upspinserver` binary this capability.

Use `setcap` command (as `root`) to do this:

```
$ setcap cap_net_bind_service=+ep /home/upspin/upspinserver
```

Note that you need to run `setcap` whenever the `upspinserver` binary is
updated.
