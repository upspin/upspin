...
The user name is an email address, but its use as an actual address occurs
only during account signup, to verify that you own that account.
Once the signup is complete, from Upspin's perspective the email address
is nothing more than a user name; you could even cancel the email
account if you chose to do so.
Even if the email account is later hijacked, the
