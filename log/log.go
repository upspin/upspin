...
// Register connects an ExternalLogger to our implicit default logger (the one
// that is pre-instantiated). This may only be called once.
...
// SetOutput sets the default logger (the one that is pre-instantiated) to
// output to w. If w is nil, the default logger is disabled.
