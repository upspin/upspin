...
func TestAllUsers(t *testing.T) {
	...
	if ok, err := packer.AllowsAllUsers(de); err != nil {
		t.Fatal(err)
	} else if !ok {
		t.Errorf("AllowsAllUsers(%q) returned false, want true", accessFile)
	}
	...
	if ok, err := packer.AllowsAllUsers(de); err != nil {
		...
		t.Errorf("AllowsAllUsers(%q) returned false, want true", file)
	}
	...
	if ok, err := packer.AllowsAllUsers(de); err != nil {
		...
		t.Errorf("AllowsAllUsers(%q) returned true, want false", file)
	}
	_, err = client2.Get(file)
