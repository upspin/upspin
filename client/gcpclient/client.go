...
// updateReadersInMeta traverses the given path (which must not be
// nil) backwards looking for the closest directory that contains an
// Access file (present as the DirEntry's Metadata.Reader). When it
// finds one, it overwrites the metadata (which must not be nil) with
// it. If not found, meta.Readers will be untouched.
func (c *Client) updateReadersInMeta(meta *upspin.Metadata, path *path.Parsed) error {
	for len(path.Elems) > 0 {
		dirEntry, err := c.context.Directory.Lookup(path.Drop(1).Path())
		if err != nil {
			return err
		}
...
func (c *Client) Put(name upspin.PathName, data []byte) (upspin.Location, error) {
	...
	if parsed == nil {
		return zeroLoc, errors.New("error parsing path %s", name)
	}
	if isAccessFile {
