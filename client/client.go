...
func (c *Client) Put(name upspin.PathName, data []byte) (*upspin.DirEntry, error) {
	...
	if err := c.addReaders(op, entry, accessEntry, packer, readers); err != nil {
		return nil, err
	}
	ss.End()
...
func (c *Client) addReaders(op string, entry, accessEntry *upspin.DirEntry, packer upspin.Packer, readers []upspin.UserName) error {
