...
func (c *Client) PutLink(oldName, linkName upspin.PathName) (*upspin.DirEntry, error) {
	...
	return c.lookup(op, entry, putLookupFn, doNotFollowFinalLink)
}
...
func (c *Client) Put(name upspin.PathName, data []byte) (*upspin.DirEntry, error) {
	...
	if packer.Packing() == upspin.EEPack {
		...
		// TODO: We need to evaluate links to get to the right server for the access check.
		if err := c.addReaders(entry, entry.Name, packer); err != nil {
			return nil, errors.E(op, err)
		}
	}

	// Record directory entry.
	return c.lookup(op, entry, putLookupFn, followFinalLink)
}
...
func (c *Client) MakeDirectory(name upspin.PathName) (*upspin.DirEntry, error) {
	...
	return c.lookup(op, &upspin.DirEntry{Name: name}, makeDirectoryLookupFn, doNotFollowFinalLink)
}

// Get implements upspin.Client.
func (c *Client) Get(name upspin.PathName) ([]byte, error) {
	const op = "client.Get"
	entry, err := c.lookup(op, &upspin.DirEntry{Name: name}, lookupLookupFn, followFinalLink)
	if err != nil {
		return nil, errors.E(op, err)
	}
	if entry.IsDir() {
...
func (c *Client) Lookup(name upspin.PathName, followFinal bool) (*upspin.DirEntry, error) {
	...
	return c.lookup(op, &upspin.DirEntry{Name: name}, lookupLookupFn, followFinal)
}
...
func (c *Client) lookup(op string, entry *upspin.DirEntry, fn lookupFn, followFinal bool) (*upspin.DirEntry, error) {
	...
	for i := 0; i < 10; i++ { // TODO: What is the right limit?
		...
		if err == nil {
			if resultEntry == nil {
				// It was a successful operation, like Put, that returns nothing.
				// Return the successful operation so the name is known.
				return entry, nil
			}
		...
		if !strings.HasPrefix(parsed.String(), string(resultPath)) {
			return nil, errors.E(op, resultPath, errors.Internal, errors.Str("link path not prefix"))
		}
	...
	return nil, errors.E(op, errors.IO, originalName, errors.Str("link loop"))
}
...
func (c *Client) Delete(name upspin.PathName) error {
	...
	_, err := c.lookup(op, &upspin.DirEntry{Name: name}, deleteLookupFn, doNotFollowFinalLink)
	return err
}
...
func (c *Client) dupOrRename(op string, oldName, newName upspin.PathName, rename bool) (*upspin.DirEntry, error) {
	entry, err := c.lookup(op, &upspin.DirEntry{Name: oldName}, lookupLookupFn, followFinalLink)
	if err != nil {
		return nil, err
	}
	if entry.IsLink() {
	...
	if !oldParsed.Drop(1).Equal(newParsed.Drop(1)) {
		// TODO: This happens before link evaluation, which is wrong.
		if err := c.addReaders(entry, entry.Name, packer); err != nil {
	...
	// Record directory entry.
	entry, err = c.lookup(op, entry, putLookupFn, followFinalLink)
	if err != nil {
		return nil, err
	}
