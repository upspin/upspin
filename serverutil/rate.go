...
func (r *RateLimiter) pass(now time.Time, key string) (bool, time.Duration) {
	...
	// that we can track at once. If so, prune back to the maximum plus a
	// bit of headroom for growth (so that we don't do this every time a
	// visitor is added).
	drop := 0
	if len(r.m) >= rateMaxVisitors {
		drop = len(r.m) - rateMaxVisitors + rateMaxHeadroom
	}
