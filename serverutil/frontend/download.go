...
func (h *downloadHandler) buildTarball(osArch string, des []*upspin.DirEntry) {
	...
	if err != nil {
		...
		// Reset latest so the tarball is rebuilt.
		h.mu.Lock()
...
func (tb *tarball) Size() string {
	return fmt.Sprintf("%.1fMB", float64(tb.size)/(1<<20))
}

// newTarball fetches des using the given Client and assembles a gzipped tar
// file containing those files, and returns the resulting tarball.
