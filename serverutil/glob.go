...
// In that case, it should also return only the DirEntry for that path.
...
// Glob executes a DirServer.Glob for the specified pattern using
// the provided LookupFunc and ListFunc to retrieve data.
func Glob(pattern string, lookup LookupFunc, ls ListFunc) ([]*upspin.DirEntry, error) {
	...
	// If there are no glob meta-characters in the pattern, do a lookup.
	if i > firstMeta || i == 0 && !isMeta(p.Elem(0)) {
		de, err := lookup(upspin.PathName(pattern))
		if de == nil {
			return nil, err
		}
