...
func doShutdown() {
	...
	shutdown.once.Do(func() {
		log.Debug.Printf("%s: Shutdown requested", op)
...
func init() {
	shutdown.barrier = sync.NewCond(&shutdown.mu)
	...
	RegisterShutdown(255, log.ShutdownLogger)
}
