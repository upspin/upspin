...
var (
	...
	currentContext = inProcessContext
)

func setup(userName upspin.UserName, context DirTestContext) (*Setup, error) {
	e := upspin.Endpoint{
		Transport: upspin.InProcess,
		NetAddr:   "",
	}
...
func TestPutTopLevelFileUsingDirectory(t *testing.T) {
	...
	s, err := setup(user, currentContext)
	if err != nil {
		t.Fatal("setup:", err)
	}
	const (
...
func init() {
	...
	if strings.HasPrefix(useGCPStore, "http") {
		...
		currentContext = gcpStoreContext
	}
}
