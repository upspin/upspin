...
var (
	...
	errNotConfigured = errors.E("GCP Directory not configured", errors.Other)
...
func (d *directory) put(op string, dirEntry *upspin.DirEntry, opts ...options) error {
	...
	if dirEntry.IsDir() && !canCreate || !dirEntry.IsDir() && !canWrite {
		return errors.E(op, dirEntry.Name, access.ErrPermissionDenied)
	}
...
func (d *directory) Glob(pattern string) ([]*upspin.DirEntry, error) {
	...
	if err != nil {
		return nil, errors.E(op, parsed.Path(), "invalid glob pattern", err)
	}
...
func (d *directory) Dial(context *upspin.Context, e upspin.Endpoint) (upspin.Service, error) {
	if e.Transport != upspin.GCP {
		return nil, errors.E(gcpDir, errors.Syntax, "unrecognized transport")
	}
	confLock.Lock()
	...
	if refCount == 0 {
		...
		refCount--
		return nil, errors.E(gcpDir, errors.Other, "internal error", errors.Str("refCount wrapped around"))
	}
