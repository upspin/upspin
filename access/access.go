...
func Parse(data []byte) (*Parsed, error) {
	...
	for s.Scan() {
		...
		log.Printf("state=%d, elems=%v", state, elems)
		switch state {
		case readers:
...
func parseLine(line []byte) (state, []path.Parsed) {
	...
	for i, c := range line {
		...
		if state == newSection {
			...
			log.Printf("Found a colon. i=%d, lastNonEmpty=%d, %c", i, lastNonEmpty, line[lastNonEmpty])
		...
		if isSpace(c) || c == ',' || i == lastChar {
			...
			log.Printf("Got token separator at %d. token=%q", i, token)
			if isAllSpace(token) {
				log.Printf("All space: %q", token)
				lastNonEmpty = i + 1
				continue
			}
			lastNonEmpty = i + 1
...
func xParse(data string) (*Parsed, error) {
	...
	for i, line := range lines {
		...
		if len(entry) != 2 {
			// we expect format: "r: bob@foo.com,..."
