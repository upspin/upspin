...
func Parse(pathName upspin.PathName, data []byte) (*Access, error) {
	...
	if numReaders > 1 && a.worldReadable {
		return nil, errors.E(op, pathName, errors.Invalid, errors.Str(`"read:all" cannot appear with other users`))
	}
	return a, nil
}
...
// The returned user name is empty unless "all" is present, in which case its text is returned.
