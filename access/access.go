...
// rightGrantedAndAccessList returns whether the requester is granted the
...
func (a *Access) Can(requester upspin.UserName, right Right, pathName upspin.PathName, load func(upspin.PathName) ([]byte, error)) (bool, error) {
	...
	// loaded groups first. The list of groups to check is implemented as a set
	// (a map) to avoid duplicates.

	var groupsToCheck groupSet
	...
	for len(group) > 0 {
		...
		for len(group) == 0 && groupsToCheck.newMembersExist() {
			parsed := groupsToCheck.nextNewMember()
...
func inGroup(requesterUserName upspin.UserName, domain string, group []path.Parsed, groupsToCheck *groupSet) bool {
...
func (a *Access) Users(right Right, load func(upspin.PathName) ([]byte, error)) ([]upspin.UserName, error) {
	...
	for {
		...
		// Loop done when the transitive closure of group membership has been exhausted, that is the entire groupSet has been expanded.
		if !groupSet.newMembersExist() {
			break
		}

		parsed := groupSet.nextNewMember()
...
// groupSet implements a set of parsed pathnames representing groups that can
// also return each element one time, without ever shrinking the set. A
// groupSet type is used because groups can appear multiple times in the
// transitive closure of the right's membership and each group should be
// expanded at most one time, but not at the time the group is first
// encountered.
//
// The newMembers elements are returned in FILO order for convenience but any
// order would satisfy the has-rights relation. In the case of method Can, the
// newMembers list is only consumed to the point where the right is ascertained
// to be granted but does require exhaustion when the right is not to be
// granted. In the case of method Users, the newMembers list is exhausted in
// order to collect all the users represented by the right.
type groupSet struct {
	set        map[path.Parsed]struct{}
	newMembers []path.Parsed
}

// newMembersExist reports whether any elements in the set haven't been returned yet.
func (s *groupSet) newMembersExist() bool {
	return len(s.newMembers) != 0
}

// add will add p to the set and to the newMembers list if not already in the set.
func (s *groupSet) add(p path.Parsed) {
	if s.set == nil {
		s.set = make(map[path.Parsed]struct{})
	}
	if _, found := s.set[p]; !found {
		s.set[p] = struct{}{}
		// No need to keep the slice sorted, FILO is fine.
		s.newMembers = append(s.newMembers, p)
	}
}

// nextNewMember returns and removes an element from the newMembers list.
// The set itself is unchanged. Caller should test that newMembers exist.
func (s *groupSet) nextNewMember() path.Parsed {
	var p path.Parsed
	p, s.newMembers = popBack(s.newMembers)
	return p
}

// popBack returns an item from the tail of the slice, and the shortened slice.
// Caller should test against being empty first.
