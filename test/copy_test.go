...
func testCopyEntries(t *testing.T, r *testenv.Runner) {
	...
	r.Entry.Sequence = -20
	if len(r.Entry.Packdata) >= 2 {
		r.Entry.Packdata[0] = 0xde
		r.Entry.Packdata[1] = 0xad
	} else {
		r.Entry.Packdata = []byte(entryPackdata)
	}
	if len(r.Entry.Blocks) > 0 && len(r.Entry.Blocks[0].Packdata) >= 2 {
		r.Entry.Blocks[0].Packdata[0] = 0xbe
		r.Entry.Blocks[0].Packdata[1] = 0xef
	} else {
	...
	r.Entry.Packdata = []byte(entryPackdata)
	// Remove all blocks, so server can't find file unless this a copy.
	r.Entry.Blocks = nil
