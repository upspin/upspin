...
type Setup struct {
	...
	// CurveName is the desired key type for the tree, e.g. "p256".
	CurveName string
...
func New(setup *Setup) (*Env, error) {
	client, context, err := innerNewUser("New", setup.OwnerName, &setup.Keys, setup.CurveName, setup.Transport)
	if err != nil {
		return nil, err
	}
...
func (e *Env) NewUser(userName upspin.UserName, keyPair *KeyPair) (upspin.Client, *upspin.Context, error) {
	return innerNewUser("NewUser", userName, keyPair, e.Setup.CurveName, e.Setup.Transport)
}
