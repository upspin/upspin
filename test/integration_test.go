...
func testAllowListAccess(t *testing.T, env *e.Env) {
	...
	// Ensure we'll not get the data
	// NOTE: this returns "empty reference" instead of permission denied because Lookup succeeded, but gave us an
	// empty location.
	// TODO: fix that in the server or add some more context to the error so it's easier to differentiate
	// List vs Read problems.
