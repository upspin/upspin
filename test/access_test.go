...
func testGroupAccess(t *testing.T, r *testenv.Runner) {
	...
	r.MakeDirectory(base)
	r.MakeDirectory(groupDir)
	r.Put(groupBffs, readerName)
	r.Put(groupFamily, familyMembers+","+readerName)
	...
	r.As(ownerName)
	r.Put(groupBffs, "# Just kidding. This is empty.")
	...
	// to the family group.
	r.As(ownerName)
	r.Put(groupBffs, "someone@else.com,family")
	r.Put(groupFamily, readerName+","+familyMembers)
