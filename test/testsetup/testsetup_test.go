...
import (
	...
	s "upspin.googlesource.com/upspin.git/test/testsetup"
...
func TestInProcess(t *testing.T) {
	...
	client, context, err := s.InProcess()
	if err != nil {
		t.Fatal(err)
	}
	err = s.AddUser(context, upspin.UserName(userName), upspin.EEp256Pack)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = s.Tree(client, verbose, testSetup)
	if err != nil {
		t.Fatal(err)
	}
