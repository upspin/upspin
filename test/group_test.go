...
func TestGroupFileMultiDir(t *testing.T) {
	...
	r.As(readerName)
	log.Printf("== readerName creating dir /Group")
	...
	r.DirLookup(groupBase + "/myclique")
	if !r.GotEntry(groupBase + "/myclique") {
		t.Fatal("meh")
	}
	log.Printf("== owner looked /Group/myclique successfully")

	// And now middleName should have access since middleName is listed
	// indirectly in readerName's team Group file.
	r.As(middleName)
	log.Printf("=== will get file for middleName")
	r.Get(file)
	if r.Failed() {
		t.Fatal(r.Diag())
	}
