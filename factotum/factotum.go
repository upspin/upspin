...
func ParsePublicKey(public upspin.PublicKey) (*ecdsa.PublicKey, string, error) {
	...
	if len(fields) != 4 { // 4 is because string should be terminated by \n, hence fields[3]==""
		return nil, "", errors.Errorf("expected keytype and two big ints, got %d %v", len(fields), fields)
	}
	keyType := fields[0]
	var x, y big.Int
	_, ok := x.SetString(fields[1], 10)
	if !ok {
		return nil, "", errors.Errorf("%s is not a big int", fields[1])
	}
	_, ok = y.SetString(fields[2], 10)
	if !ok {
		return nil, "", errors.Errorf("%s is not a big int", fields[2])
	}
