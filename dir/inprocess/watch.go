...
type listener struct {
	...
	done     <-chan struct{} // Fromt he Watch method; signals termination.
...
func (l *listener) sendAll(events []upspin.Event) bool {
	for _, event := range events {
		...
		if err != nil {
			// Shouldn't happen.
			log.Info.Printf("parse error in sendAll for %q: %v", event.Entry.Name, err)
			continue
		}
...
// inTree reports whether the name is in the subtree starting at the root.
func inTree(root, name path.Parsed) bool {
	if name.User() != root.User() || name.NElem() < root.NElem() {
		return false
	}
	for i := 0; i < root.NElem(); i++ {
		if name.Elem(i) != root.Elem(i) {
			return false
		}
	}
	return true
...
func (l *listener) sendTree(name upspin.PathName) bool {
	...
	if err != nil {
		// Shouldn't happen.
		log.Info.Printf("parse error in sendTree for %q: %v", name, err)
	}
	...
	go func() {
		...
		case 0:
			...
			if !l.sendAll(eventsSoFar) {
				log.Printf("Watch %q could not send all initial events", root)
				return
			}
		case -1:
