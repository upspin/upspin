...
func (s *Service) Glob(pattern string) ([]*upspin.DirEntry, error) {
	...
	for i := 0; i < parsed.NElem(); i++ {
		...
		for _, ent := range this {
			...
			// intermediate step (directory) if it's matched by a pattern,
			//and for the final entry always.
			if isGlobPattern(elem) || lastElem {
	...
	for _, entry := range next {
		isRoot := entry.Name == upspin.PathName(user)
		if isRoot {
			// Need a / on the root.
			entry.Name += "/"
		}
		...
		if parent := parsed.Drop(1).Path(); !isRoot && checkedPrefix != parent {
