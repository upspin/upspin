...
func (s *server) whichAccess(p path.Parsed) (*upspin.DirEntry, error) {
	...
	// TODO: check the cache for an access dir entry for this path.
...
func (s *server) loadPath(name upspin.PathName) ([]byte, error) {
	...
	// TODO: relax this constraint, possibly using Client in a go routine.
	// See thread titled "Remote Group files" for context.
	if p.User() != s.userName {
		return nil, errors.E(op, name, errors.Str("can't currently fetch other user's Access/Group files"))
	}
	tree, err := s.loadTreeFor(p.User())
	if err != nil {
		return nil, errors.E(op, err)
