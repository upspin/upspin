...
func (s *server) takeSnapshot() error {
	...
	name := upspin.PathName(fmt.Sprintf("%s/%s/%s", user, path.SnapshotPrefix, date))
	...
	log.Printf("=== root.Blocks: %d", len(root.Blocks))
	...
	return tree.PutSpecial(p, &newEntry)
}
...
func (s *server) safeMkDir(name upspin.PathName) (*upspin.DirEntry, bool, error) {
	log.Printf("Trying to make: %q", name)
