// Copyright 2016 The Upspin Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package server

import (
	"strings"
	"testing"

	"upspin.io/dir/server/tree"
	"upspin.io/upspin"
)

func xTestMakeSnapshotPath(t *testing.T) {
	s := newDirServerForTesting(t, userName)

	name := upspin.PathName(userName + "/snapshot/2016/05/09")
	entry, err := s.makeSnapshotPath(name)
	if err != nil {
		t.Fatal(err)
	}
	if got, want := entry.Name, name; got != want {
		t.Errorf("entry.Name = %q, want = %q", got, want)
	}

	// Try to make it again.
	entry, err = s.makeSnapshotPath(name)
	if err != nil {
		t.Fatal(err)
	}
	if got, want := entry.Name, name+".1"; got != want {
		t.Errorf("entry.Name = %q, want = %q", got, want)
	}
}

func xTestSnapshot(t *testing.T) {
	s := newDirServerForTesting(t, userName)

	// Take a few snapshots manually.
	for i := 0; i < 5; i++ {
		err := s.takeSnapshot()
		if err != nil {
			t.Fatal(err)
		}
	}
	// Glob inside all the snapshots.
	entries, err := s.Glob(userName + "/" + tree.SnapshotPrefix + "/*/*/*")
	if err != nil {
		t.Fatal(err)
	}
	expectedEntries := []string{
		"Access",
		"snapshot",
		"some_new_file.txt",
	}
	for i, e := range entries {
		if i < 2 { // because the first two are from the test above.
			continue
		}
		name := string(e.Name) + "/*"
		t.Logf("Globbing: %q", name)
		ssEntries, err := s.Glob(name)
		if err != nil {
			t.Fatal(err)
		}
		if len(ssEntries) != len(expectedEntries) {
			t.Fatalf("len(ssEntries) = %d, want = %d", len(ssEntries), len(expectedEntries))
		}
		for j, ne := range ssEntries {
			if !strings.HasSuffix(string(ne.Name), "/"+expectedEntries[j]) {
				t.Errorf("ne.Name = %q, want suffix = %q", ne.Name, expectedEntries[j])
			}
		}
	}

	// Lookup something inside the snapshot.
	name := entries[2].Name + "/some_new_file.txt"
	de, err := s.Lookup(name)
	if err != nil {
		t.Fatal(err)
	}
	if got, want := de.Name, name; got != want {
		t.Errorf("de.Name = %q, want = %q", got, want)
	}
}
