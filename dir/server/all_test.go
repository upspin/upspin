...
func TestHasRight(t *testing.T) {
	...
	checkAccess := func(p path.Parsed, right access.Right, want bool) error {
	...
	err = checkAccess(p, access.List, true) // owner always has read access.
	if err != nil {
		t.Fatal(err)
	}
	err = checkAccess(p, access.Read, true) // owner always has read access.
	if err != nil {
		t.Fatal(err)
	}
	err = checkAccess(p, access.Write, false)
	if err != nil {
		t.Fatal(err)
	}
	err = checkAccess(p, access.Create, false)
	if err != nil {
		t.Fatal(err)
	}
}
