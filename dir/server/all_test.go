...
func TestMakeRoot(t *testing.T) {
	...
	checkDirEntry(t, "TestMakeRoot", deLookup, de)
...
func TestPut(t *testing.T) {
	...
	checkDirEntry(t, "TestPut", de2, de)
}
...
func TestMakeDirectory(t *testing.T) {
	...
	checkDirEntry(t, "TestMakeDirectory", de2, de)
}
...
func checkDirEntry(t *testing.T, testName string, got, want *upspin.DirEntry) {
	if got.Name != want.Name {
		t.Errorf("got.Name = %q, want = %q", got.Name, want.Name)
	}
	if got.Writer != want.Writer {
		t.Errorf("got.Writer = %q, want = %q", got.Writer, want.Writer)
	}
	if got.Attr != want.Attr {
		t.Errorf("got.Attr = %v, want = %v", got.Attr, want.Attr)
	}
	if got.Packing != want.Packing {
		t.Errorf("got.Packing = %q, want = %q", got.Packing, want.Packing)
	}
}
