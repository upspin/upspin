...
func (s *server) put(op string, p path.Parsed, entry *upspin.DirEntry, opts ...options) (*upspin.DirEntry, error) {
	...
	if err == upspin.ErrFollowLink {
		log.Printf("====== put: got ErrFollowLink on %q", p)
	...
	if errors.Match(errNotExist, err) {
		...
		if !canCreate {
			log.Printf("======= put: user %q can't create new entry: %q", s.userName, p)
			return nil, s.errPerm(op, p, o)
		}
	...
	if err == upspin.ErrFollowLink {
		log.Printf("==== put: tree.put returned ErrFollowLink on p=%q", p)
		return entry, err
	}
	if err != nil {
