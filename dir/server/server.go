...
type server struct {
	...
	// The Storage backend in which to store roots.
...
func New(cfg upspin.Config, options ...string) (upspin.DirServer, error) {
	...
	if storageBackend == "" {
		// If no storage backend was provided for the roots, use
		// disk-based storage in the same location as the logs.
		rootDir := filepath.Join(logDir, "root")
		storageBackend = "Disk"
		storageOpts = []storage.DialOpts{storage.WithOptions("basePath=" + rootDir)}
	}
