...
var (
	...
	// Access to userTrees must be protected by the user lock (see
	// userlock.go). Concurrent access for different users is okay as
	// the LRU is thread-safe.
	userTrees = cache.NewLRU(1000)
...
type server struct {
	...
	// userContext holds the context of the user on behalf of whom this
	// server is serving. Currently only the user name is needed.
	// TODO: should we use just the user name here instead?
	userContext upspin.Context
...
func (s *server) put(op string, p path.Parsed, entry *upspin.DirEntry) (*upspin.DirEntry, error) {
	if p.Path() != entry.Name {
		return nil, errors.Str("path name is not clean")
	}
	...
	if err != nil {
		...
		// As expected, entry not found, continue.
	} else {
...
func (s *server) Dial(ctx upspin.Context, e upspin.Endpoint) (upspin.Service, error) {
	...
	if e.Transport != upspin.Cloud {
		return nil, errors.E(op, errors.Invalid, errors.Str("transport must be Cloud"))
	}
	...
	cp.userContext = ctx.Copy()
	return &cp, nil
}

// Configure implements upspin.Service.
func (s *server) Configure(options ...string) error {
	// TODO: configure log directory option.
	return nil
}
...
func (s *server) loadTreeFor(user upspin.UserName) (tree.Tree, error) {
	...
	if val, found := userTrees.Get(user); found {
		if tree, ok := val.(tree.Tree); ok {
			return tree, nil
		}
		// This should never happen because we only store type tree.Tree in the userTree.
		return nil, errors.E(op, user, errors.Internal, errors.Str("tree not of type Tree"))
	}
	...
	// If user has root, we're all set.
...
func (s *server) createRoot(op string, p path.Parsed) (*upspin.DirEntry, error) {
	if s.userContext.UserName() != p.User() {
		// Can only create root for the authenticated user.
		return nil, errors.E(op, errors.Invalid, s.userContext.UserName(),
	...
	// Add to the cache and return.
	userTrees.Add(p.User(), tree)

	log.Printf("Created root for user %q", p.User())
	return de, nil
}
