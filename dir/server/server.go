...
func (s *server) Lookup(name upspin.PathName) (*upspin.DirEntry, error) {
	...
	defer m.Done()
	switch string(name) {
	case string(s.userName) + "/garbageList": // Ugly hack to return tree references via error.
		return nil, s.garbageList(name, false, o)
	case string(s.userName) + "/garbageListVerbose":
...
func (s *server) garbageListUser(buf *bytes.Buffer, user upspin.UserName, verbose bool) error {
	...
	refs, err := tree.RefList(t, verbose)
	if err != nil {
		return errors.E(op, err)
	}
