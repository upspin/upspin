...
// shutdown is called when the server is being forcefully shutdown.
func (s *server) shutdown() {
	it := s.userTrees.NewIterator()
	for k, v, next := it.GetAndAdvance(); !next; k, v, next = it.GetAndAdvance() {
		user := k.(upspin.UserName)
		tree := v.(*tree.Tree)
		err := tree.Close()
		if err != nil {
			log.Error.Printf("Error closing tree for user %s: %s", user, err)
		}
	}
}
