...
func New(ctxt upspin.Context, options ...string) (upspin.DirServer, error) {
	...
	for _, opt := range options {
		o := strings.Split(opt, "=")
		if len(o) != 2 {
			return nil, errors.E(op, errors.Invalid, errors.Errorf("invalid option format: %s", opt))
		}
		switch o[0] {
		case "userCacheSize", "accessCacheSize":
			cacheSize, err := strconv.ParseInt(o[1], 10, 32)
			if err != nil {
				return nil, errors.E(op, errors.Invalid, err)
			}
			if cacheSize < 1 {
				return nil, errors.E(op, errors.Invalid, errors.Errorf("%s: cache size too small: %d", o[0], cacheSize))
			}
			switch opt {
			case "userCacheSize":
		...
		case "logDir":
			logDir = o[1]
		default:
			return nil, errors.E(op, errors.Invalid, errors.Errorf("unknown option %s", o[0]))
		}
	}
	if logDir == "" {
