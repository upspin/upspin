...
func (le *LogEntry) unmarshal(r *checker) error {
	...
	data, err := ioutil.ReadAll(&io.LimitedReader{R: r, N: entrySize})
	if err != nil {
		return errors.E(op, errors.IO, errors.Errorf("reading %d bytes from entry: %s", entrySize, err))
	}
	if len(data) != int(entrySize) {
		return errors.E(op, errors.IO, errors.Errorf("read fewer bytes than needed: %d; wanted %d", len(data), entrySize))
	}
