...
func newPooledChecker(r io.Reader) *checker {
	return &checker{
		r:       r,
		chksum:  checksumSalt,
		buf:     pool.Get().([]byte),
...
// done signals the end of life for checker. It is mostly intended for
// reclaiming buffer space when using a sync pool.
func (c *checker) done() {
	if c.hasPool {
		pool.Put(c.buf)
	}
