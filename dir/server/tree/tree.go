...
// TODOs: fine-grained locking; metrics; performance tuning.
// Watch API TODOs:
// - Remove dead watchers preemptively.
...
func New(context upspin.Context, log *Log, logIndex *LogIndex) (*Tree, error) {
	...
	err = t.recoverFromLog(lastProcessed)
	if err != nil {
...
func (t *Tree) Put(p path.Parsed, de *upspin.DirEntry) (*upspin.DirEntry, error) {
	...
	t.sendWatcherNotifications(watchers, logEntry, t.log.LastOffset())
	return de, nil
}
...
func (t *Tree) Watch(p path.Parsed, order int64, done <-chan struct{}) (<-chan *upspin.Event, error) {
	...
	// We're ready to send notifications from this point on. To fulfill the
	// request from 'order' and forward, we must have a copy of
	// the tree, so we can keep this alternative tree unmodified in memory
	// while we serve the current blocks to the caller.
	...
	if order >= 0 {
		err = clone.recoverFromLog(order)
		if err != nil {
			return nil, errors.E(op, err)
		}
	}
	// Traverse the transitive closure for the cloned tree

	// Now march the logs forward till log.LastOffset().
...
// node, loadPath also returns all watchers p or anywhere in p's ancestors.
// t.mu must be held.
func (t *Tree) loadPath(p path.Parsed) (*node, []watcher, error) {
	...
	node := t.root
	// Keep track of all ancestor watchers to p, starting with those in the
	// root.
...
func (t *Tree) sendWatcherNotifications(watchers []watcher, logEntry *LogEntry, offset int64) {
	for _, watcher := range watchers {
		watcher.ch <- &upspin.Event{
			Order:  offset,
...
// operations. It can only be called from New or with t.mu held.
func (t *Tree) recoverFromLog(position int64) error {
	const (
