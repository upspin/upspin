...
// PutDir puts a new DirEntry representing a directory already loaded with kids
// in its DirBlocks into the tree at the point represented by path dst.
func (t *Tree) PutDir(dst path.Parsed, de *upspin.DirEntry) (*upspin.DirEntry, error) {
	...
	// Check if dst exists and is empty.
	parent, err := t.loadPath(dst.Drop(1))
	if err == upspin.ErrFollowLink { // encountered a link along the path.
		return &parent.entry, err
	}
	if errors.Match(errors.E(errors.NotExist), err) {
		return nil, errors.E(op, dst.Path(), errors.NotExist, errors.Str("destination parent must exist"))
	}
	if err != nil {
		return nil, errors.E(op, err)
	}
	if parent.entry.IsLink() {
		return &parent.entry, upspin.ErrFollowLink
	}
	if len(parent.kids) > 0 || len(parent.entry.Blocks) > 0 {
		return nil, errors.E(op, dst.Path(), errors.NotEmpty, errors.Str("destination must be empty"))
	}
	/*
		// Create a new path by appending the last element of the source path
		// to the destination path.
		p, err := path.Parse(de.Name)
		if err != nil {
			return nil, errors.E(op, err)
		}
		newPath, err := path.Parse(path.Join(dst.Path(), p.Elem(p.NElem()-1)))
		if err != nil {
			return nil, errors.E(op, err)
		}
	*/
	// Create the hard link dir.
	//de.Name = dst.Path() //
	node, err := t.put(dst /*newPath*/, de)
	if err == upspin.ErrFollowLink {
	...
	log.Printf("=== Created node for dst: %+v", *node)
	err = t.loadKids(node)
	if err != nil {
		return nil, errors.E(op, err)
	}
