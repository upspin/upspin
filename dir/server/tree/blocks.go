...
func (t *Tree) store(n *node) error {
	...
	for _, kid := range n.kids {
		...
		// Check whether there are any Blocks with empty blocks or
		// locations that are non-empty dirs or files.
		for _, b := range kid.entry.Blocks {
			if len(kid.kids) != 0 && (len(b.Location.Reference) == 0 || b.Size == 0) {
				return errors.E(kid.entry.Name, errors.Internal, errors.Str("empty directoy block when there exist kid blocks"))
			}
		}
