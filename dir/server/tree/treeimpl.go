...
// All fields are required. Log manipulates the log on behalf of the tree for
// a user. LogIndex is used by Tree to track the most recent changes stored in
...
func (t *tree) markDirty(p path.Parsed) error {
	...
	for i := 0; i < p.NElem(); i++ {
		...
		// only their parents (directories).
		if !n.entry.IsDir() {
			continue
		}
