...
func (t *tree) delete(p path.Parsed) (*node, error) {
	...
	parent, err := t.loadPath(parentPath)
	if err != nil {
		return nil, errors.E(deleteOp, err)
	}
	...
	if len(node.kids) > 0 {
		...
		return nil, errors.E(deleteOp, errors.NotEmpty, p.Path())
	}
...
func (t *tree) recoverFromLog() error {
	...
	recovered := 0
	for {
		log.Debug.Printf("Going to recover from log...")
		...
		for _, logEntry := range replay {
			...
			switch logEntry.Op {
			case Put:
				log.Debug.Printf("Going to put de: %v", de.Name)
				err = t.put(p, &de)
			case Delete:
				log.Debug.Printf("Going to delete %q", p.Path())
				_, err = t.delete(p)
			default:
