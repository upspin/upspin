...
func removeDeadWatchers(n *node) {
	...
	for i := 0; i < len(n.watchers); i++ {
		...
		closed := n.watchers[i].events == nil || doneCh == nil
		if !closed {
...
func isPrefixPath(path, prefix upspin.PathName) bool {
	p := string(path)
	pref := string(prefix)
	return strings.HasPrefix(p, pref) && strings.Count(p, "/") >= strings.Count(pref, "/")
}
