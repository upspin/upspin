...
type watcher struct {
	...
	// active indicates whether the watcher is active (1) or inactive (0).
...
func (w *watcher) sendEvent(logEntry *LogEntry, offset int64) error {
	...
	case <-time.After(watcherTimeout):
		...
		w.sendErrorAndClose(errTimeout)
		return errTimeout
	}
	return nil
}

func (w *watcher) sendErrorAndClose(err error) {
	e := &upspin.Event{
		Error: err,
	}
	select {
...
// IsWatching reports whether this watcher is actively watching for updates.
...
func (w *watcher) close() {
	active := atomic.LoadInt32(&w.active)
	if active == 0 {
		log.Error.Printf("dir/server/tree.Watch: closing closed watcher")
		return
	}
...
func removeDeadWatchers(n *node) {
	...
	for i := 0; i < len(n.watchers); i++ {
		...
		watching := n.watchers[i].IsWatching()
		if watching {
