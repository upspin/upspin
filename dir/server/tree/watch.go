...
func (w *watcher) sendCurrentAndWatch(clone, orig *Tree, p path.Parsed, offset int64) {
	...
	if err != nil && !errors.Match(errNotExist, err) {
		log.Error.Printf("tree.sendCurrentAndWatch: error loading path: %s", err)
		w.sendError(err)
		w.close()
		return
	}
	...
	if err == nil {
		...
		if err != nil {
			log.Error.Printf("tree.sendCurrentAndWatch: : error traversing tree: %s", err)
			w.sendError(err)
			w.close()
			return
		}
	...
	orig.mu.Unlock()
	if err != nil {
		log.Error.Printf("tree.sendCurrentAndWatch: : error adding watcher: %s", err)
		w.sendError(err)
		w.close()
		return
	}
