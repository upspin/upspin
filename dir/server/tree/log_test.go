...
func TestAppendRead(t *testing.T) {
	...
	if got, want := logger.User(), user; got != want {
		t.Errorf("logger.User = %s, want = %s", got, want)
	}
	...
	if got, wantAtLeast := logger.LastOffset(), uint64(minEntrySize*10); got < wantAtLeast {
		t.Errorf("LastOffset = %d, want > %d", got, wantAtLeast)
	}
	// Read LogEntries back in two passes.
	entries, nextOffset, err := logger.Read(0, 6)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wantAtLeast := uint64(minEntrySize * 6); nextOffset < wantAtLeast {
		t.Errorf("nextOffset = %d, want > %d", nextOffset, wantAtLeast)
	}
	// Read more. Attempt to go past the EOF
	entries, nextOffset, err = logger.Read(nextOffset, 32)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got, want := string(entries[0].Entry.Name), "foo@bar.com/hello6"; got != want {
		t.Errorf("entries[0].Entry.Name = %s, want = %s", got, want)
	}
	...
	if got, want := string(entries[3].Entry.Name), "foo@bar.com/hello9"; got != want {
		t.Errorf("entries[3].Entry.Name = %s, want = %s", got, want)
	}
}
