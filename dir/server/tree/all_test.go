...
func TestRebuildFromLog(t *testing.T) {
	...
	de = newDirEntry("/dir0", isDir, cfg)
	err = tree.Put(de)
	if err != nil {
		t.Fatal(err)
	}
	de = newDirEntry("/dir0/file_in_dir.txt", !isDir, cfg)
	err = tree.Put(de)
	if err != nil {
		t.Fatal(err)
	}
