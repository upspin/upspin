...
func BenchmarkReadAt(b *testing.B) {
	...
	user, err := Open(userName, dir, nil)
	if err != nil {
		b.Fatal(err)
	}
...
func TestReadRotatedLog(t *testing.T) {
	...
	// Open logs for user.
	user2, err := Open(user.name, user.directory, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	user3, err := Open(user.name, user.directory, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer user3.Close()
...
func TestRotateLogAndTruncate(t *testing.T) {
	...
	user, err := Open("bob@example.com", dir, nil)
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 10; i++ {
...
func TestIndex(t *testing.T) {
	...
	user, err := Open("foo@bar.com", dir, store)
	if err != nil {
		t.Fatal(err)
	}
	...
	<-store.onPut
	b, err := store.Storage.Download(user.rootFileRef())
	if err != nil {
		t.Fatal(err)
	}
...
func TestVersion0Logs(t *testing.T) {
	...
	user, err := Open("user@example.com", dir, nil)
