...
// changeAccess is called whenever an Access file, acc, changes. It
// updates the access pointer from any file that is a descendant of
// acc's directory and which doesn't have a more specific Access file.
//
...
func (l *clog) changeAccess(access *clogEntry, add bool) {
	...
	for {
		...
		if parsedEntry.Equal(parsedAccessDir) {
			entry.access = newval
			// We may no longer be able to read or list
			// elements of the other user's directory.
			// Mark it as incomplete since we won't
			// necessarily see directory changes.
			if notMe {
