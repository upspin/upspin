...
// since, after its access file changes, we don't know if the entry is
// still accessible.
//
...
func (l *clog) changeAccess(e *clogEntry, add bool) {
	...
	// notMe is true if e's directory belongs to another user.
	parsed, err := path.Parse(e.name)
	if err != nil {
		return
	}
	...
	for {
		...
		if !strings.HasPrefix(string(ne.name), string(dirName)) {
			// This file is not a descendant of e's directory.
			continue
		}
		...
		// ne is a descendent of e's directory and is not known to use
		// a more specific access file. Mark its access file as unknown
		// and, if the directory is not our own, invalidate ne since
