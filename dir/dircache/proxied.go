...
type proxiedDir struct {
	...
	// supported is true if watch is supported.
	supported bool
}
...
// proxyFor saves the endpoint and makes sure it is being watched. It returns
// true of this is cachable.
func (p *proxiedDirs) proxyFor(name upspin.PathName, ep *upspin.Endpoint) bool {
	p.Lock()
	defer p.Unlock()
	if p.closing {
		return false
	...
	if d.die == nil {
		// Don't start a watcher for snapshots and remember
		// not to cache anything that might change.
		// TODO(p): once snapshots start returning ErrNotSupported for Watch,
		// just set d.supported to true and wait for the first Watch to fix it.
		_, suffix, _, err := user.Parse(u)
		d.supported = err == nil && suffix != "snapshot"
		if d.supported {
