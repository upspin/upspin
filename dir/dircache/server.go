...
type server struct {
	// cfg is a config that points directly to the uncached server.
	cfg upspin.Config

	// loopbackCfg is a config that points back at this dircache.
	// It exists to allow dircache to use itself and the associated
	// storecache when reading access and group files.
	loopbackCfg upspin.Config
...
func New(cfg, loopbackCfg upspin.Config, cacheDir string, maxLogBytes int64, flushBlock func(upspin.Location)) (upspin.DirServer, error) {
...
func (s *server) Put(entry *upspin.DirEntry) (*upspin.DirEntry, error) {
	...
	if err != nil {
		...
		// match the server.
		if granted {
			log.Error.Printf("access refused but we predicted granted: %s, %v, %s", name, *entry, err)
		}
		return de, err
	}
	...
	// match the server.
	if !granted {
		log.Error.Printf("access granted but we predicted refused: %s, %s", name, accErr)
	}
	return de, err
}
...
func (s *server) Delete(name upspin.PathName) (*upspin.DirEntry, error) {
	...
	if granted {
		if err != nil {
			log.Error.Printf("access refused but we predicted granted: %s, %s", name, err)
		}
	} else {
		if err == nil {
			log.Error.Printf("access granted but we predicted refused: %s, %s", name, accErr)
...
// This must be called while holding locks since it recurses into the cacheserver.
...
//
// This must be called while holding locks since it recurses
...
func (s *server) readAndParseAccess(de *upspin.DirEntry) (*access.Access, error) {
	...
	contents, err := clientutil.ReadAll(s.loopbackCfg, de)
	if err != nil {
		return nil, err
	}
...
func (s *server) load(name upspin.PathName) ([]byte, error) {
	...
	return clientutil.ReadAll(s.loopbackCfg, de)
}
