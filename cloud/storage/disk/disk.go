...
func guaranteeNewEncoding(base string) error {
	...
	if _, err := os.Stat(plusDir); err != nil {
		...
		format := "Base directory %[1]q uses a deprecated path encoding.\n" +
			...
			"To update, choose a new location (not the existing one) and run:\n" +
			"\tgo run upspin.io/cloud/storage/disk/convert.go -old=%[1]q -new=<new-location>\n" +
			"Then restart server with option basePath=<new-location>\n"
