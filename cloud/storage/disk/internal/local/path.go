...
// with directory. The tree is never longer than 5 levels.
...
func Path(base, ref string) string {
	...
	const (
		...
		// The longest path is therefore base plus numDirs plus a final element.
		numDirs  = 5
		numElems = 1 + numDirs + 1 // Base plus directories plus file name.
	)
