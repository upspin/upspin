...
func New(opts *storage.Opts) (storage.Storage, error) {
	...
	if keyData, ok := opts.Opts[privateKeyData]; !ok {
		...
		ts, err := google.JWTAccessTokenSourceFromJSON(b, scope)
		if err != nil {
			return nil, errors.E(op, errors.Invalid, errors.Errorf("unable to set up service account: %s", err))
		}
		client = oauth2.NewClient(gContext.Background(), ts)
	}
