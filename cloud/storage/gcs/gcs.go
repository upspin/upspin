...
func (gcs *gcsImpl) Rename(oldRef, newRef string) (newRefLink string, error error) {
	acl := string(gcs.defaultWriteACL)
	res, err := gcs.service.Objects.Copy(gcs.bucketName, oldRef, gcs.bucketName, newRef, &gcsBE.Object{}).DestinationPredefinedAcl(acl).Do()
	if err != nil {
		return "", err
	}
	err = gcs.Delete(oldRef)
	return res.MediaLink, err
}
