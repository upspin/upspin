...
func ListenAndServe(ready chan<- struct{}, serverName, addr string, opt *Options) {
	...
	// TODO(ehg)   m.HostPolicy = autocert.HostWhitelist("dir.upspin.io")
	...
	if metadata.OnGCE() {
		...
		cache, err := initCache(bucket, serverName)
		if err != nil {
		...
		s := &http.Server{
			...
			// TODO(ehg) Handler:      http.HandlerFunc(?),
		}
	...
	if file := opt.LetsEncryptCache; file != "" {
		...
		m.Cache = autocert.DirCache(file) // TODO err?
		config = &tls.Config{
...
func initCache(bucket, suffix string) (cache autocertCache, err error) {
	// TODO(ehg)  Don't we need to do some construction first time?
