...
func TestVerifyBodyComparison(t *testing.T) {
	...
	expectedLogMessage := "Body contents mismatch. Number of mismatched bytes: 1"
	if newT.LogMessages[0] != expectedLogMessage {
		t.Fatalf("Verify logged incorrect error. Expected %v, got %v", expectedLogMessage, newT.LogMessages[0])
	}
}
