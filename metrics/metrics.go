...
// Done ends the metric and any un-ended span and saves it to stable storage. Further use of the metric or any of
...
func (m *Metric) Done() {
	...
	case saveQueue <- m:
		// Sent
	default:
		log.Error.Printf("Metric channel is full. Dropping metric %s (perhaps there are no savers registered?)", m.name)
	}
}
