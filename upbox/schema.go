...
type Schema struct {
	...
	// It is read from the session file found in the upbox output directory, if
	// one is set.
	session session
}
...
func SchemaFromYAML(doc string) (*Schema, error) {
	...
	if sc.Dir != "" {
		if err := sc.session.from(sc.Dir); err != nil {
	...
	for i, s := range sc.Servers {
		...
		var err error
		if s.addr, err = sc.session.address(s.Name); err != nil {
...
// sessionFile specifies the name of the file holding information about
// a previously started session.
const sessionFile = "session"

// session holds information that helps restore a previously started
// upbox session.
type session struct {
...
// from attempts to read session information from the given directory.
func (s *session) from(dir string) error {
...
// save attempts to save the current session data to the given directory.
...
// setAddress sets the address that the given server is listening on during
// this session.
func (s *session) setAddress(server, addr string) {
	if s.ServerAddr == nil {
...
// address returns the address that the server with the given name should listen on
// for this session.
func (s *session) address(name string) (string, error) {
	...
	s.setAddress(name, "localhost:"+port)
...
func (sc *Schema) Start() error {
	...
	if err := sc.session.save(sc.Dir); err != nil {
		return err
	}
