...
func (sc *Schema) Start() error {
	...
	for _, u := range sc.Users {
		...
		// TODO(ehg) Avoid this duplicate config by merging with loop below.
		configKeygen := filepath.Join(dir, "keygenconfig")
		if err := ioutil.WriteFile(configKeygen, []byte("secrets: "+dir), 0644); err != nil {
			return err
		}
		var buf bytes.Buffer
		keygen := exec.Command("upspin", "-config="+configKeygen, "keygen")
