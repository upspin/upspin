...
// Period and in Google anything after the + is ignored.
// We accept these but do not follow the ignore rule.
//
func UserAndDomain(userName upspin.UserName) (user string, domain string, err error) {
	...
	trail := domain
	if lastPeriod == len(domain)-1 {
		trail = trail[:lastPeriod]
	}
	if period+2 >= len(trail) {
