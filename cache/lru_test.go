...
func TestPeek(t *testing.T) {
	...
	if k, v := c.PeekLRU(); k != nil || v != nil {
		t.Errorf("LRU = %q, %q; want nil, nil", k, v)
	}
	if k, v := c.PeekMRU(); k != nil || v != nil {
	...
	if k, v := c.PeekLRU(); k != "k1" || v != "v1" {
		t.Errorf("LRU = %q, %q; want k1, v1", k, v)
	}
	if k, v := c.PeekMRU(); k != "k2" || v != "v2" {
		t.Errorf("MRU = %q, %q; want k2, v2", k, v)
	}

	c.Get("k1")
	if k, v := c.PeekLRU(); k != "k2" || v != "v2" {
		t.Errorf("LRU = %q, %q; want k2, v2", k, v)
	}
	if k, v := c.PeekMRU(); k != "k1" || v != "v1" {
