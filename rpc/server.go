...
func hashUser(magic, user, host, now string) []byte {
	l := make([]byte, 4)
	b := make([]byte, len(magic)+len(user)+len(host)+len(now)+3*4)
	copy(b, magic)
	binary.BigEndian.PutUint32(l, uint32(len(user)))
	b = append(b, l...)
	b = append(b, user...)
	binary.BigEndian.PutUint32(l, uint32(len(host)))
	b = append(b, l...)
	b = append(b, host...)
	binary.BigEndian.PutUint32(l, uint32(len(now)))
	b = append(b, l...)
	b = append(b, now...)
	h := sha256.Sum256(b)
	return h[:]
