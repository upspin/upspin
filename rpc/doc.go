...
In its first request to the server, the client presents a signed authentication
request as a series of HTTP headers with key 'Upspin-Auth-Request'.
The header values are, in order:
       the user name,
       the server host name,
       the current time, and
       the R and S parts of an upspin.Signature (in separate headers).
The signature is generated by calling the client's upspin.Factotum's Sign method
on a hash of the user name, host name, and time. (TODO: describe this in detail.)
The server validates the signature and, if valid, it creates a session and
returns an authentication token in the 'Upspin-Auth-Token' response header.

In subsequent requests, the client presents the authentication token to the
