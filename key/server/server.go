...
func (s *server) fetchUserEntry(op string, name upspin.UserName, sp *metric.Span) (*userEntry, error) {
	log.Debug.Printf("%s: %s", op, name)
	sp2 := sp.StartSpan("storage.Download")
	...
	sp2 = sp.StartSpan("json.Unmarshal")
	var entry userEntry
