...
func (r *remote) Lookup(name upspin.UserName) (*upspin.User, error) {
	...
	u := &upspin.User{
		Name:      upspin.UserName(resp.UserName),
		Dirs:      proto.UpspinEndpoints(resp.Dirs),
		Stores:    proto.UpspinEndpoints(resp.Stores),
		PublicKey: upspin.PublicKey(resp.PublicKey),
	}
	return u, nil
}
