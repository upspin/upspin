...
func (c *cache) create(h *handle) error {
	if h.n.cf != nil {
		return errors.E(errors.IO, fmt.Errorf("create of an open file"))
	}
...
func (c *cache) open(h *handle, flags fuse.OpenFlags) error {
	...
	for i := 0; i < len(locations); i++ {
		...
		if packer == nil {
			finalErr = errors.E(errors.IO, fmt.Errorf("no packer found"))
			continue
		}
		clearLen := packer.UnpackLen(n.f.context, data, de)
		if clearLen < 0 {
			finalErr = errors.E(errors.IO, fmt.Errorf("unpack len < 0"))
			continue
		}
