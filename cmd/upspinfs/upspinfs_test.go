...
func mount() error {
	...
	if !found {
		for i := 1; i < 5; i++ {
			...
			testConfig.mountpoint = fmt.Sprintf("/tmp/upspinfstest%d", i)
			umountHelper(testConfig.mountpoint)
...
func writeFile(fn string, buf []byte) (*os.File, error) {
	...
	if n != len(buf) {
		f.Close()
		return nil, fmt.Errorf("%s: wrote %d bytes, expected %d", fn, n, len(buf))
	}
	return f, nil
}
...
func readAndCheckContents(t *testing.T, fn string, buf []byte) {
	...
	if n != len(buf) {
		t.Fatalf("%s: read %d bytes, expected %d", fn, n, len(buf))
	}
	for i := range buf {
		if buf[i] != rbuf[i] {
			t.Fatalf("%s: error at byte %d", fn, i)
		}
	}
}

func mkFile(t *testing.T, fn string, buf []byte) {
	f, err := writeFile(fn, buf)
	if err != nil {
		t.Fatal(err)
	}
...
func notExist(t *testing.T, fn, event string) {
	if _, err := os.Stat(fn); err == nil {
		t.Fatal(fmt.Printf("%s: should not exist after %s", fn, event))
	}
}
...
func TestFile(t *testing.T) {
	...
	wf, err = writeFile(fn, buf[:len(buf)/2])
	if err != nil {
		t.Fatal(err)
	}
	if err := wf.Close(); err != nil {
...
func testSymlink(t *testing.T, link, rooted, relative string, contents []byte) {
	...
	if val != relative {
		t.Fatal(fmt.Printf("%s: Readlink returned %s, expected %s:]", link, val, relative))
	}
	remove(t, link)
