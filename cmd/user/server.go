...
import (
	...
	"upspin.googlesource.com/upspin.git/cmd/stats"
...
var _ stats.Stats = (*serverStatus)(nil)

func (uc *serverStatus) Print(w io.Writer) {
	if uc.err != nil {
		fmt.Fprint(w, "error: %s", uc.err)
	}
	fmt.Fprintf(w, "num registered users = %d\n", uc.numUsers)
	fmt.Fprintf(w, "num lookups last hour = %d", uc.numLookups)
}

func (uc *serverStatus) Update() {
	users, err := uc.cloudClient.ListDir("")
	if err == nil {
		uc.numUsers = len(users)
	}
	uc.addLookup(0)
	uc.err = err
}
...
func (u *userServer) statusHandler(w http.ResponseWriter, r *http.Request) {
	stats.OutputReport(nil, w)
}
...
func main() {
	...
	stats.New("user counter", userStats)
	http.HandleFunc("/status", u.statusHandler)
