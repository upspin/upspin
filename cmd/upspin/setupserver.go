...
It registers the server user for the given domain with the key server, copies
the configuration files from $HOME/upspin/deploy/example.com to the
upspinserver and restarts it, puts the upspin@example.com/Group/Writers file,
and makes the root for the calling user.
...
`
	...
	s.parseFlags(fs, args, help, "setupserver [-where=$HOME/upspin/deploy] [-writers=user,...] <domain>")
	if fs.NArg() != 1 {
		fs.Usage()
	}
	...
	if err == nil {
		...
		fmt.Printf("Put %q to the key server.\n", cfg.User)
	}
	...
	fmt.Printf("Configured upspinserver.\n")
