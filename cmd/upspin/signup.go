...
func makeSignedRequest(cfg upspin.Config) (string, error) {
	...
	w("name", string(cfg.UserName()))
	w("dir", string(cfg.DirEndpoint().NetAddr))
	w("store", string(cfg.StoreEndpoint().NetAddr))
	w("key", string(cfg.Factotum().PublicKey()))
	sig, err := cfg.Factotum().Sign(h.Sum(nil))
	if err != nil {
		return "", err
	}
