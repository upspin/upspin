...
`
	...
	fs.String("secretseedfile", "", "`file` containing a 128 bit secret seed in proquint format")
...
func (s *State) keygenCommand(fs *flag.FlagSet) {
	...
	secretFlag := subcmd.StringFlag(fs, "secretseedfile")
...
func createKeys(curveName, secretFlag string) (public, private, secretStr string, err error) {
	...
	if len(secretFlag) > 0 {
		...
		// Fall through and use secretFlag
		if len((secretStr)) != 47 || (secretStr)[5] != '-' {
