...
func (s *State) setupwriters(args ...string) {
	const help = `
Setup-writers creates or updates a Writers file for the given project.
The listed names are given access to write to the project's store server and
create their own root on the directory server.
...
This command is designed to operate on projects created with setup-domain.
`
