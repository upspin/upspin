...
func main() {
	...
	storeAuthServer := grpcauth.NewServer(storeCtx, nil)
	proto.RegisterStoreServer(grpcServer, storeserver.New(storeCtx, store, storeAuthServer, upspin.NetAddr(*storeAddr)))
	dirAuthServer := grpcauth.NewServer(dirCtx, nil)
	proto.RegisterDirServer(grpcServer, dirserver.New(dirCtx, dir, dirAuthServer, upspin.NetAddr(*dirAddr)))
