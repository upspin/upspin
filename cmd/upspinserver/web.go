...
// whichAccess returns Access entry for path name, handling broken links.
func (s *web) whichAccess(name upspin.PathName) (*upspin.DirEntry, error) {
	...
	for {
		...
		if err == upspin.ErrFollowLink {
			// If we get link error, go back up the paths.
			// NOTE: Link must be broken as we already looked up.
