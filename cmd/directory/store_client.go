...
// Get fetches the contents of a reference as pointed to by a location. It implicitly binds to the endpoint in the location.
...
func (s *storeClient) requestAndReadResponseBody(op string, req *http.Request) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, newDirError(op, "", fmt.Sprintf("store server error", resp.StatusCode))
	}
	// Check the content type
	answerType := resp.Header.Get(netutil.ContentType)
	if !strings.HasPrefix(answerType, "application/json") {
		return nil, newDirError(op, "", fmt.Sprintf("invalid response format: %v", answerType))
	}
