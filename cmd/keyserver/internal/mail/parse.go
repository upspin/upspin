...
func ParseMail(data url.Values) (string, string, error) {
	...
	if idx := strings.Index(from, "<"); idx >= 0 {
		...
		if end < idx {
			return "", "", errors.E(op, errors.Invalid, errors.Str("invalid email format"))
		}
		from = from[idx+1 : end]
	}
	...
	if len(fields) != 2 {
		return "", "", errors.E(op, errors.Internal, errors.Errorf("DKIM information not parseable"))
	}
...
Outer:
	for s.Scan() {
		...
		case keyPreamble:
			...
			if err != nil {
				return "", "", "", errors.E(op, errors.Invalid, errors.Str("missing public key preamble"))
			}
			state = key
		case key:
		...
		case signature:
			...
			if n != 1 || err != nil {
				return "", "", "", errors.E(op, errors.Invalid, errors.Str("missing signature"))
			}
			break Outer
		}
	}
