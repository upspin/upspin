...
// To use this load tester, ensure there is no client caching by commenting out
// the Global key cache in key/remote/remote.go, in its init function.
//
// To test the best case scenario, where the key server is expected to have all
// users cached in memory, use the -working_set flag with a small number and
// -preauth. For example:
//
// ./keyloadtester -jobs 250 -working_set=10 -preauth
//
...
//
// To test the most adversarial scenario possible, do not set -working_set or
// -preauth flags.
//
...
var (
	...
	n          = flag.Int("n", 1000, "number of requests per job")
	preauth    = flag.Bool("preauth", true, "whether to exclude the first request, which does more work, from the measurements")
	workingSet = flag.Int("working_set", 0, "if >0 requests the same `working_set` users over and over")
...
func keyServerOrDie() upspin.KeyServer {
