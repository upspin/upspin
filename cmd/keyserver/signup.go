...
const signupNotifyAddress = "upspin-sendgrid@google.com"
...
func (m *signupHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		errorf(http.StatusBadRequest, "invalid name: %v", err)
		return
	}
	...
	const (
		subject = "Upspin signup confirmation"
		noHTML  = ""
	)
	...
	err = m.mail.Send(signupNotifyAddress, serverName, "New signup: "+string(u.Name),
		fmt.Sprintf("%s signed up on %s", u.Name, time.Now().Format(time.Stamp)), noHTML)
	if err != nil {
