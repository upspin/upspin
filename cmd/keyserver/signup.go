...
func (m *signupHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if ok, wait := m.rate.Pass(string(u.Name)); !ok {
	...
	fmt.Fprintln(body, "Follow this link to complete the Upspin process:")
	fmt.Fprintln(body, signupURL)
	fmt.Fprintln(body, "\nIf you were not expecting this message, please delete it.")
