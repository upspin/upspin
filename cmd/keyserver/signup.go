...
var (
	...
	emailUserName = flag.String("email_user", "", "Username of incoming email provider authenticates with the keyserver")
	emailPassword = flag.String("email_pwd", "", "Password of incoming email provider authenticates with the keyserver")
...
func incomingHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = createUser(u)
	if err != nil {
		sendMailOrLog(from, fmt.Sprintf("Failure creating Upspin user %q", userName), err.Error())
		return
	}
