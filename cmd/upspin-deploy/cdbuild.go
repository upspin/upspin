...
func cdbuild(dir, projectID, name string) error {
	...
	if err := setupBucket(ctx, projectID, hc, stagingBucket); err != nil {
		if gerr, ok := err.(*googleapi.Error); ok {
			if gerr.Code == 403 {
				// HACK(cbro): storage returns a 403 if billing is not enabled.
				return fmt.Errorf("Could not set up Cloud Storage bucket. It's possible billing is not enabled. Root cause: %v", err)
			}
		}
...
func setupBucket(ctx context.Context, projectID string, hc *http.Client, bucket string) error {
