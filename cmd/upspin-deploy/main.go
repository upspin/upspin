...
func (c *Config) createNetwork() error {
	...
	return okReason("alreadyExists", c.waitGlobalOp(svc, op, err))
}
...
func (c *Config) deleteNetwork() error {
	...
	err = svc.Firewalls.List(c.Project).Filter("network eq .*/"+c.networkName()+"$").Pages(ctx, func(list *compute.FirewallList) error {
		for _, fw := range list.Items {
			...
			err = okReason("notFound", c.waitGlobalOp(svc, op, err))
			if err != nil {
				return err
			}
		}
	...
	return okReason("notFound", c.waitGlobalOp(svc, op, err))

}

func (c *Config) waitGlobalOp(svc *compute.Service, op *compute.Operation, err error) error {
...
func (c *Config) createAddresses() error {
	...
	for _, s := range c.servers() {
		...
		if err := okReason("alreadyExists", opError(op, err)); err != nil {
			return err
		}
	}
	return nil
...
func (c *Config) deleteAddresses() error {
	...
	for _, s := range c.servers() {
		...
		if err := okReason("notFound", opError(op, err)); err != nil {
			return err
		}
	}
	return nil
...
func (c *Config) createDisks() error {
	...
	return okReason("alreadyExists", opError(op, err))
...
func (c *Config) deleteDisks() error {
	...
	return okReason("notFound", opError(op, err))
}
