...
type node struct {
	...
	handles    map[*handle]bool // handles associated with this node.
}
...
func (n *node) Create(context xcontext.Context, req *fuse.CreateRequest, resp *fuse.CreateResponse) (fs.Node, fs.Handle, error) {
	...
	h := allocHandle(nn)
	h.dirty = true
	h.fname = filepath.Join(n.f.cacheDir, fmt.Sprintf("new.%d", n.id))
	var err error
	...
	if err := nn.dirPut([]byte{0}); err != nil {
...
func (n *node) openFile(context xcontext.Context, req *fuse.OpenRequest, resp *fuse.OpenResponse) (fs.Handle, error) {
	...
	for i := 0; i < len(locations); i++ {
		...
		if data, locs, err = store.Get(loc.Reference); err != nil {
			finalErr = eio("%s Get %q ref %q file %q", err, h.n.uname, loc.Reference, h.fname)
			continue
		}
		if len(locs) > 0 {
		...
		if packer == nil {
			finalErr = eio("couldn't lookup %q ref %q file %q", h.n.uname, loc.Reference, h.fname)
			continue
		}
		clearLen := packer.UnpackLen(n.f.context, data, &de.Metadata)
		if clearLen < 0 {
			finalErr = eio("couldn't unpack %q ref %q file %q", h.n.uname, loc.Reference, h.fname)
			continue
		}
		cleartext := make([]byte, clearLen)
		rlen, err := packer.Unpack(n.f.context, cleartext, data, &de.Metadata, h.n.uname)
		if err != nil {
			finalErr = eio("%s unpacking %q ref %q file %q", err, h.n.uname, loc.Reference, h.fname)
			continue
		}
		cleartext = cleartext[:rlen]
		// Save a copy of the cleartext in the local file system.
		if h.file, err = os.Create(h.fname); err != nil {
			return nil, eio("%s creating %q ref %q file %q", err, h.n.uname, loc.Reference, h.fname)
		}
		if wlen, err := h.file.Write(cleartext); err != nil || rlen != wlen {
			return nil, eio("%s writing %q ref %q file %q", err, h.n.uname, loc.Reference, h.fname)
		}
		n.Lock()
...
func (n *node) Setattr(context xcontext.Context, req *fuse.SetattrRequest, resp *fuse.SetattrResponse) error {
	...
	if req.Valid.Size() {
		// Truncate any existing cached files for this node.  The file will be truncated on
		// the store if one of the open handles is writable.
		n.Lock()
		n.attr.Size = req.Size
		willSet := false
		for h := range n.handles {
			h.file.Truncate(int64(req.Size))
			if !h.flags.IsReadOnly() {
				willSet = true
				h.dirty = true
			}
		}
		...
		// If noone has it open for writing, make sure we read it in, resize and put.
		if !willSet {
			oreq := &fuse.OpenRequest{Flags: fuse.OpenWriteOnly}
			oresp := &fuse.OpenResponse{}
			xh, err := n.openDir(context, oreq, oresp)
			if err != nil {
				return err
			}
			h := xh.(*handle)
			h.dirty = true
			h.file.Truncate(int64(req.Size))
			h.Release(context, &fuse.ReleaseRequest{})
		}
	}
	if req.Valid.Mtime() {
		// Set the modify time.
...
// free implements fs.HandleWriter.Release.  This corresponds to a user close and, if the file is dirty,
...
func (h *handle) Release(context xcontext.Context, req *fuse.ReleaseRequest) error {
	...
	// No matter what happens the handle is no longer valid on return.
	defer h.free()
	if !h.dirty {
		return nil
	}
	...
	if err = h.n.dirPut(cleartext); err != nil {
		return eio("%s Directory.Put(%s)", err, h.n.uname)
	}
	return nil
}

func (n *node) dirPut(cleartext []byte) error {
	ue, err := n.f.users.lookup(n.user)
	if err != nil {
