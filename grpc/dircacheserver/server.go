...
func New(ctx upspin.Context, ss grpcauth.SecureServer) (proto.DirServer, error) {
	...
	clog, err := openLog(ctx, ospath.Join(homeDir, "upspin/dircache"), 120*time.Second)
	if err != nil {
		return nil, err
	}
	return &server{
...
// TODO(p): figure out what to do here, need another log entry type...
func (s *server) WhichAccess(ctx gContext.Context, req *proto.DirWhichAccessRequest) (*proto.EntryError, error) {
	...
	return op.entryError(dir.WhichAccess(name))
}
