...
var goodLogEntries = []clogEntry{
	{request: lookupReq, name: upspin.PathName("x@y.com/a/b/c"), error: noError, entries: []*upspin.DirEntry{&dirEnt}},
	{request: lookupReq, name: upspin.PathName("x@y.com/a/b/c"), error: notExistError},
	{request: lookupReq, name: upspin.PathName("x@y.com/a/b/c"), error: errFollowLink, entries: []*upspin.DirEntry{&dirEnt}},
	{request: globReq, name: upspin.PathName("x@y.com/a/b/c"), error: noError, entries: []*upspin.DirEntry{&dirEnt, &dirEnt, &dirEnt}},
}
...
func TestLogFile(t *testing.T) {
	...
	for _, name := range names {
		good := clogEntry{request: putReq, name: upspin.PathName(name), entries: []*upspin.DirEntry{&dirEnt}}
		l.logRequest(good.request, good.name, good.error, good.entries[0])
	}
	for _, name := range names {
		good := clogEntry{request: putReq, name: upspin.PathName(name), entries: []*upspin.DirEntry{&dirEnt}}
		e := l.lookup(upspin.PathName(name))
		if err := compare(&good, e); err != nil {
			t.Errorf("%v: %v", good, err)
		}
	}
