...
const (
	byteLimit = 10 * 1000 * 1000 * 1000 // This is an approximate limit. We can be one ref larger.
...
func New(ctx upspin.Context, ss grpcauth.SecureServer) proto.StoreServer {
	...
	return &server{
		ctx:          ctx,
		SecureServer: ss,
		cache:        newCache(path.Join(homeDir, "upspin/storecache"), byteLimit, refLimit),
	}
}
...
func (s *server) Get(ctx gContext.Context, req *proto.StoreGetRequest) (*proto.StoreGetResponse, error) {
	...
	data, locs, err := s.cache.get(s.ctx, upspin.Reference(req.Reference), store)
	if err != nil {
		op.log(err)
		return &proto.StoreGetResponse{Error: errors.MarshalError(err)}, nil
	}
