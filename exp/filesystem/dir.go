...
func (s dirServer) listDir(dir upspin.PathName) ([]*upspin.DirEntry, error) {
	parsed, err := path.Parse(dir)
	if err != nil {
		return nil, err
	}
	if ok, err := s.can(access.List, parsed); err != nil {
		...
		return nil, errors.E(dir, errors.Private)
	}
	names, err := filepath.Glob(filepath.Join(s.root, parsed.FilePath(), "*"))
	if err != nil {
		return nil, err
	}
	...
	for _, file := range names {
		...
		parsed, err := path.Parse(s.upspinPathFromLocal(file))
		if err != nil {
			return nil, err
		}
