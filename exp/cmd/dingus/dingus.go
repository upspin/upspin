...
// Command dingus serves an Upspin tree containing a series of boxes (files)
// containing Schrödinger's cats. The cats inside the boxes are in the
...
// is revealed.
//
// The purpose of this program is to demonstrate the implementation of an
// Upspin DirServer and StoreServer that serve dynamic content.
//
...
var (
	accessBytes   = []byte("read,list:all\n")
	accessName    = "Access"
	accessRef     = upspin.Reference(accessName)
...
func newServer(ep upspin.Endpoint, cfg upspin.Config) (*server, error) {
	...
	var err error
	s.accessEntry, err = s.pack(accessName, accessBytes)
	if err != nil {
		return nil, err
	}
...
func (s *server) pack(filePath string, data []byte) (*upspin.DirEntry, error) {
	...
	// Since we are using EEIntegrityPack we don't need to return the
	// resulting ciphertext, as it's equivalent to the cleartext.
	if _, err := bp.Pack(data); err != nil {
		return nil, err
	}
...
func (s *dirServer) listDir(name upspin.PathName) ([]*upspin.DirEntry, error) {
	...
	for ; n < len(s.boxes); n++ {
		box := &s.boxes[n]
		de := box.DirEntry.Copy()
		de.MarkIncomplete()
		des = append(des, de)
	}

	// The final, closed box.
	des = append(des, s.closedBox(n))
...
var (
	boxName = "box"
	boxRE   = regexp.MustCompile(`^` + boxName + `(\d+)$`)
)
...
func matchBox(filePath string) int {
	m := boxRE.FindStringSubmatch(filePath)
	if len(m) == 0 {
		return -1
	}
	i, _ := strconv.ParseInt(m[1], 10, 32)
	if i < 0 || fmtBox(int(i)) != m[0] {
		return -1
	}
	return int(i)
}
...
var (
	...
	dead  = []byte("A dead cat.\n")
	alive = []byte("A live cat.\n")
)
