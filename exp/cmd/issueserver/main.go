...
var (
	...
	dataDir        = flag.String("data-dir", defaultDataDir, "Local directory in which to write maintner files")
	defaultDataDir = filepath.Join(os.Getenv("HOME"), "var", "issueserver")
)
...
// $HOME/.github-issue-toke of the format "user:token".
...
// server's issue map, and returns the resulting DirEntry. If the issue has is
...
// directory returns a directory DirEntry for name.
...
// link returns a link DirEntry for name that points to the given issue.
