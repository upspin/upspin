...
func pickPort() (net.Listener, int) {
	...
	var err error
	for _, port := range []int{6666, 6667, 7777, 9999, 4444, 3333, 2222, 1111} {
...
func (s *server) DoATrump(ctx gContext.Context, req *prototest.DoATrumpRequest) (*prototest.DoATrumpResponse, error) {
	...
	s.t.Fatal("iteration %d: invalid request %q", s.iteration, req.PeopleDemand)
...
func startClient(port int) {
	...
	conn, err := grpcauth.NewGRPCClient(upspin.NetAddr(fmt.Sprintf("localhost:%d", port)), allowSelfSignedCert)
	if err != nil {
		log.Fatal(err)
	}
