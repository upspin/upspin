...
func (ac *AuthClientService) NewAuthContext() (ctx gContext.Context, opt grpc.CallOption, finishAuth func(error) error, err error) {
	...
	finishAuth = func(err error) error {
		...
		// If talking to a proxy, make sure it is running as the same user.  The
		// time in the original client auth request serves as a challenge for the server.
		if ac.isProxy() {
			...
			if err := ac.verifyServerUser(msg); err != nil {
				log.Error.Printf("client can't verify user: %s", err)
