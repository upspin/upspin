...
// HandlerFunc is a type used by HTTP handler functions that want to use a HandlerInterface for authentication.
type HandlerFunc func(authHandler HandlerInterface, w http.ResponseWriter, r *http.Request)

// HandlerInterface is used by HTTP servers to authenticate Upspin users.
type HandlerInterface interface {
	...
	// IsAuthenticated returns whether the connection is authenticated to a particular user. Calls to User will return a valid user name.
	IsAuthenticated() bool

	// Err returns whether there was any error in authentication.
	Err() error

	// Handle is the chained handler function that should be used to register an authenticated handler. See example above.
...
// Config holds the configuration parameters for an instance of HandlerInterface.
type Config struct {
	// A Lookup function to lookup user keys. Most callers can safely user context.User.Lookup here.
	Lookup func(userName upspin.UserName) ([]upspin.Endpoint, []upspin.PublicKey, error)

	// If set to true, allows unauthenticated connections to go through and it's the callers responsibility to check IsAuthenticated.
	AllowUnauthenticatedConnections bool

	// TODO: set preferred cipher method.
}

// AuthHandler implements a HandlerInterface that ensures cryptography-grade authentication.
...
var _ HandlerInterface = (*AuthHandler)(nil)

// NewHandler creates a new instance of a HandlerInterface according to the given config, which must not be changed subsequently  by the caller.
func NewHandler(config *Config) HandlerInterface {
...
func (ah *AuthHandler) doAuth(w http.ResponseWriter, r *http.Request) error {
	...
	if ah.config.Lookup == nil {
		return errors.New("no Lookup function available")
	}
