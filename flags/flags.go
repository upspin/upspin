...
func Enable(vars ...interface{}) error {
	for i, v := range vars {
		...
		case *string:
			...
			case &Project:
				flag.StringVar(v, "project", "", "The GCP project name, if any.")
			case &ConfigFile:
				flag.StringVar(v, "configfile", "", "Name of file with config parameters with one key=value per line")
			default:
				unknown = true
			}
		case *logFlag:
