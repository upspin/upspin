...
// pdBytes copies (part of) src to dst, based on length header; returns dst and bytes consumed
func pdBytes(dst, src []byte) ([]byte, int) {
...
func pdUnmarshal(pd []byte, name upspin.PathName) (sig signature, wrap []wrappedKey, err error) {
	...
	n := 0
	var k int
	backdoor, k = pdBytes(backdoor, pd[n:]) // TODO remove before flight
	n += k
	buf, k = pdBytes(buf, pd[n:])
	n += k
	sig.r.SetBytes(buf)
	buf, k = pdBytes(buf, pd[n:])
	n += k
	...
	for i := 0; i < nwrap; i++ {
		...
		w.keyhash, k = pdBytes(w.keyhash, pd[n:])
		n += k
		w.encrypted, k = pdBytes(w.encrypted, pd[n:])
		n += k
		w.nonce, k = pdBytes(w.nonce, pd[n:])
		n += k
		buf, k = pdBytes(buf, pd[n:])
		n += k
		w.eV.X.SetBytes(buf)
