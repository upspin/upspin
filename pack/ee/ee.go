...
var allUsersKeyHash = factotum.KeyHash(upspin.AllUsersKey)
...
func (ee ee) Unpack(cfg upspin.Config, d *upspin.DirEntry) (upspin.BlockUnpacker, error) {
	...
	for _, w := range pd.wrap {
		all := bytes.Equal(allUsersKeyHash, w.keyHash)
...
func (ee ee) Share(cfg upspin.Config, readers []upspin.PublicKey, packdataSlice []*[]byte) {
	...
	for j, d := range packdataSlice {
		...
		for i, w := range pd.wrap {
			...
			if bytes.Equal(allUsersKeyHash, w.keyHash) {
				dkey = w.dkey
			} else {
...
func (ee ee) AllowsAllUsers(d *upspin.DirEntry) (bool, error) {
	const op = "pack/ee.AllowsAllUsers"
