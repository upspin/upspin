...
func init() {
	pack.Register(eePack{"factotum", "keyserver"})
}
...
// Signature should be ECDSA but use sha256 as a placeholder for now   TODO
...
func (e eePack) Pack(ciphertext, cleartext []byte, meta *upspin.Metadata, name upspin.PathName) (int, error) {
	...
	for i, user := range usernames {
		hash16 := cksum(user)                                // TODO call keyserver
...
func (e eePack) Unpack(cleartext, ciphertext []byte, meta *upspin.Metadata, name upspin.PathName) (int, error) {
	...
	var mismatch error
	dkey := make([]byte, aesKeylen)
	sig, wrap := pdUnmarshal(meta.PackData, name) // TODO update pdUnmarshal
	parsed, err := path.Parse(name)               // first wrapped key is for owner = parsed.User
	if err != nil {
		return 0, fmt.Errorf("EEPack: filename does not parse: %s", parsed.User)
	}
	mykey := e.keyserver + "/" + string(parsed.User) // TODO replace by keyserver.Lookup
	hash16 := cksum(mykey)
	for _, w := range wrap {
		if hash16 != w.Hash {
			continue
...
func pdMarshal(sig Signature, wrap []WrappedKey) []byte {
	...
	if err != nil {
		panic("binary.Write sig failed")
	}
	err = binary.Write(buf, binary.BigEndian, wrap)
	if err != nil {
		panic("binary.Write wrap failed")
	}
	return buf.Bytes()
}
