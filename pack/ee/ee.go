...
func (ee ee) Unpack(cfg upspin.Config, d *upspin.DirEntry) (upspin.BlockUnpacker, error) {
	...
	writerRawPubKey, err := internal.GetPublicKey(cfg, writer)
	if err != nil {
	...
	me := cfg.UserName()
	rawPublicKey, err := internal.GetPublicKey(cfg, me)
	if err != nil {
		return nil, errors.E(op, d.Name, err)
	}
...
func (ee ee) Name(cfg upspin.Config, d *upspin.DirEntry, newName upspin.PathName) error {
	...
	ownerRawPubKey, err := internal.GetPublicKey(cfg, owner)
	if err != nil {
...
func pdMarshal(dst *[]byte, sig, sig2 upspin.Signature, wrap []wrappedKey, cipherSum []byte) error {
	...
	for _, w := range wrap {
		n += internal.PutBytes((*dst)[n:], w.keyHash)
		n += internal.PutBytes((*dst)[n:], w.dkey)
		n += internal.PutBytes((*dst)[n:], w.nonce)
		n += internal.PutBytes((*dst)[n:], w.ephemeral.X.Bytes())
		n += internal.PutBytes((*dst)[n:], w.ephemeral.Y.Bytes())
	}
	n += internal.PutBytes((*dst)[n:], cipherSum)
	*dst = (*dst)[:n]
...
func pdUnmarshal(pd []byte) (sig, sig2 upspin.Signature, wrap []wrappedKey, hash []byte, err error) {
	...
	for i := 0; i < nwrap; i++ {
		...
		n += internal.GetBytes(&w.keyHash, pd[n:])
		n += internal.GetBytes(&w.dkey, pd[n:])
		n += internal.GetBytes(&w.nonce, pd[n:])
		n += internal.GetBytes(&buf, pd[n:])
