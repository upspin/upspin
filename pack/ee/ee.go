...
func (c common) eePack(ctx *upspin.Context, ciphertext, cleartext []byte, meta *upspin.Metadata, pathname upspin.PathName) (int, error) {
	...
	// Set up readers. The writer of a file is always an owner.
	usernames := append([]upspin.UserName{ctx.UserName}, meta.Readers...)
	rawPublicKey, err := c.publicKey(ctx, ctx.UserName)
	if err != nil {
		return 0, err
	}
	...
	for i, u := range usernames {
		...
		pubK, err := c.parsePublicKey(rawPubK)
		if err != nil {
			log.Printf("parsing pub key for user %s: %s", u, err)
			if firstErr == nil {
				firstErr = err
			}
			continue
		}
		wrap[i], err = c.aesWrap(pubK, privateKey, dkey)
		if err != nil {
			return 0, err
		}
	}
