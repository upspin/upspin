...
func (c common) Share(ctx *upspin.Context, readers []upspin.PublicKey, packdata []*[]byte) {
	...
	for i, pub := range readers {
		...
		copy(hash[i][:], keyHash(pubkey[i])) // TODO Is this the best way to copy 32 bytes from a slice to an array?
	}
	...
	for i, d := range packdata {
		...
		var dkey []byte
		oldmap := make(map[keyHashArray]wrappedKey)
		sig, oldwrap, err := c.pdUnmarshal(*d)
		for i, w := range oldwrap {
			var h keyHashArray
			copy(h[:], w.keyHash) // TODO same question as above
			oldmap[h] = oldwrap[i]
