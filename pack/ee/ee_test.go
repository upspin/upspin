...
func testPackRetargetAndUnpack(t *testing.T, ctx *upspin.Context, packer upspin.Packer, name, newName upspin.PathName, text []byte) {
	// First pack.
	d := &upspin.DirEntry{}
	d.Name = name
	cipher := packBlob(t, ctx, packer, d, text)
	...
	// Retarget to newName.
	if err := packer.Retarget(ctx, d, newName); err != nil {
...
func TestRetarget256(t *testing.T) {
	...
	ctx, packer := setup(user, packing)
	testPackRetargetAndUnpack(t, ctx, packer, name, newName, []byte(text))
}
