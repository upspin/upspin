...
type packdata struct {
	...
	wrap []wrappedKey
	// hash is a hash of the the blocks.
	hash []byte
}
...
func (pd *packdata) Marshal(dst *[]byte) error {
	...
	// hash
	n += packutil.PutBytes((*dst)[n:], pd.hash)
...
func (pd *packdata) Unmarshal(b []byte) error {
	...
	// hash
	pd.hash = make([]byte, sha256.Size)
	n += packutil.GetBytes(&pd.hash, b[n:])
	if pd.hash == nil {
		return errors.Str("file hash is required")
	}
