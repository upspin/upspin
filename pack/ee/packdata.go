...
// ee.Marshal copies byte arrays to the destination in the order declared in
// the packdata and wrappedKey struct definitions, prefixed with length using
// binary.PutVarint.
func packdataLen(nwrap int) int {
	// marshalBufLen is an upper bound on size of big.Int used by ee
	vi := binary.MaxVarintLen64
	// nWrappedKey is size of a single encoded wrappedKey

	nWrappedKey := vi + sha256.Size            // keyHash
	nWrappedKey += vi + aesKeyLen + gcmTagSize // dkey
	nWrappedKey += vi + gcmStandardNonceSize   // nonce
	nWrappedKey += 2 * (vi + marshalBufLen)    // ephemeral

	n := 4 * (vi + marshalBufLen) // (R,S) for (sig, sig2)
	n += vi                       // len(wrap)
	n += nwrap * nWrappedKey
	n += vi + sha256.Size // blockSum
