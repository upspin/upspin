...
func (base) ApplyFlags(string) error        { return nil }
...
func asFlags(v interface{}, m map[string]map[string]string) error {
	...
	for k, v := range cmds {
		cmd, err := asString(k)
		if err != nil {
			return errors.E(errors.Invalid, errors.Errorf("bad command %v %q", v, err))
		}
		flags, ok := v.(map[interface{}]interface{})
		if !ok {
			return errors.E(errors.Invalid, errors.Errorf("cmd %q bad value %v", cmd, v))
		}
		fm := make(map[string]string)
		for k, v := range flags {
			flag, err := asString(k)
			if err != nil {
				return errors.E(errors.Invalid, errors.Errorf("cmd %q bad flag %s", cmd, err))
			}
			val, err := asString(v)
			if err != nil {
				return errors.E(errors.Invalid, errors.Errorf("cmd %q flag %q %s", cmd, flag, err))
			}
			fm[flag] = val
		}
		m[cmd] = fm
...
// ApplyFlags updates any flag that is still at its default value. It will
// apply all the flags possible and return the last error seen.
func (cfg cfgFlags) ApplyFlags(cmd string) error {
	const op = "config.ApplyFlags"
	flags := cfg.flags[cmd]
	if flags == nil {
		return nil
	}
